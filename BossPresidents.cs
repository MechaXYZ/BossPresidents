...
namespace BossPresidents
{
    [UKPlugin("gov.BossPresidents","Boss Presidents", "1.0.0", "Replaces the prime fights and Gabriel's first fight with the current and last two presidents.\nOriginal concept by: https://www.youtube.com/@spunklord5000", true, false)]
    public class Presidents : UKMod
    {
        ...
        {
            Debug.Log("Boss Presidents starting");

            // load the asset bundle
            var assembly = Assembly.GetExecutingAssembly();
            var resourceName = "BossPresidents.Resources.bosspresidents";
            using (Stream stream = assembly.GetManifestResourceStream(resourceName)){
                PresidentsAssetBundle =  AssetBundle.LoadFromStream(stream);
            }

            // start harmonylib to swap assets
            harmony = new Harmony("gov.BossPresidents");
            harmony.PatchAll();
        }
        ...
        internal class Patch01
        {
            static void Postfix(MinosPrime __instance){
                Debug.Log("Replacing Minos' voicelines");
